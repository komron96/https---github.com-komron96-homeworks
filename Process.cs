    ...
    public List<string> FimiQueryExecution(NpgsqlConnection conn)
    {
        ...
        using (var cmd = new NpgsqlCommand(sqlQuery, conn))
        {
            using (var reader = cmd.ExecuteReader())
            {
                ...
                object amountObject = reader.GetValue(1); // Используйте GetDecimal для считывания числовых значений
                decimal currency = reader.GetDecimal(2);
                
                string amountString = (amountObject == DBNull.Value) ? "N/A" : amountObject.ToString();

                fimiList.Add(date);
                fimiList.Add(amountObject.ToString()); // Преобразуйте числовое значение в строку
